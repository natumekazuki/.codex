...
- AIエージェントが作業時に参照する規約・タスク指示を作成・見直す際は、対象エージェントが目的を達成するために必要な判断材料を、必要な場面で提供する。作業は目的・対象・完了条件と適用される契約・権限で定義する。指示の作成・見直しでは、対象エージェントが既に利用できる情報の不足と改善する判断を明確にし、文章量だけでなく、持ち込む前提・調査・確認・承認待ちの負担で追加・維持を評価する。不足には元の指示の明確化、参照条件・提供先の調整を優先し、個別の禁止・例外・再通知は、それらでは解消できない具体的な不足を補う場合に用いる。
...
- 実装変更前に、対象repositoryの規約文書（AGENTS.md、README、またはそこから明示された文書）で、変更が影響する維持対象と残件の管理先を確認する。不足・矛盾が変更可否や残件の引継ぎを左右する場合は、その判断に依存する変更・完了判断を止め、根拠・推奨案・草案を示して確認する。独立した変更や調査・reviewまで一律に止めず、未調査を安全の根拠にしたり、未合意の保護対象を独断で確定・廃止したりしない。
