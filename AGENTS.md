...
- 動作する機能を早期に統合し、後続タスクの依存を解消する。機能実装、review、後追い修正、統合、公開の完了を分け、review指摘の全件修正を機能実装の完了・merge・通常のリリースの条件にしない。
...
- 統合ブロッカーは、対象環境での統合後のビルド・起動・必要な組込みの不成立、担当機能または統合先の主要動作の不成立、後続に必要な合意済み契約の不成立とする。具体的な根拠のある重大なデータ破壊・情報漏えい・権限逸脱も停止対象とする。停止には成立しない動作・依存・安全境界と根拠を示し、優先度、今回の差分起因、変更範囲内であること、修正の容易さだけでは止めない。
...
- 追加確認はブロッカー修正、新たな変更、競合解消、必須check失敗、具体的な安全上の懸念に必要な範囲へ限定する。再開・base更新だけで確認済み範囲を再探索しない。同じブロッカーへの対応が収束しない場合は統括またはユーザーへ未解決事項と必要な判断を報告し、無限反復や未解決の合格扱いに進まない。
...
- 統括は権限と必須条件を満たした変更を統合し、必要な統合後確認が成立したら依存先を着手可能にする。後追い修正の起動・完了を、元機能の統合や後続の開始条件にしない。修正は統合後のコードを基点とする別タスク・別セッションへ割り当て、空き枠と編集競合を調整する。
- 修正担当は指定された不具合を解消し、途中で見つけた別の非ブロッカーは分離する。修正対象がなければ待機または別の明示タスクへ移り、稼働維持のためのバグ探索を続けない。
