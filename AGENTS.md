...
- 指示は、受け手が現在の目的を達成するために必要な判断材料を、必要な場面で提供する。作業は目的・対象・完了条件と適用される契約・権限で定義する。指示の作成・見直しでは、受け手が既に利用できる情報の不足と改善する判断を明確にし、文章量だけでなく、持ち込む前提・調査・確認・承認待ちの負担で追加・維持を評価する。不足には元の指示の明確化、参照条件・提供先の調整を優先し、個別の禁止・例外・再通知は、それらでは解消できない具体的な不足を補う場合に用いる。
