...
- Python／TypeScript／C#のtestを新規追加・意味変更する場合、`review-test-value`がactiveな必須gateとして登録されていれば、task開始時のbaseから有効な審査経路の完了条件を満たす。削除・移設もSkillの解消確認を省かない。candidate状態のworkflowは明示起動だけで、repositoryの必須gateにしない。審査やcheckを通すためだけに契約を弱めない。
...
- 委譲する調査・データ取得・範囲が明確な実装・検証はLunaを優先する。SolはLunaで解消できなかった具体的な問題、専門審査の指定、またはユーザーの明示指定に限る。親が設計判断と統合を担い、子には対象・必要な文脈・期待結果だけを渡す。専門審査のmodelと手順は該当Skillに従い、Lunaの形式的な試行や不要な分業を増やさない。
