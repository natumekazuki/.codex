...
- 履歴として残す文書はADRと各repositoryの`docs/releases/`のリリースノートに限定する。ADRは削除せず、置換・撤回時は本文を保持して適用対象外と後継判断を明記する。それ以外の文書・ソースコード・コメント・設定・testには現行の情報だけを置き、古い定義、廃止済みの実装・説明、移行経緯、完了済み計画を残さない。変更に伴い不要になった情報は同じ論理変更で削除し、注記・別名・退避先で温存しない。リリースノートから過去の詳細を参照する場合は、その内容を含むGit commitの完全ハッシュへ固定したリンクを使い、参照のために旧文書を現行treeへ残さない。詳細は条件付き参照の設計判断・文書化に従う。
