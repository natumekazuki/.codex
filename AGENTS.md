...
- 委譲する調査・データ取得・範囲が明確な実装・検証はLunaを優先する。SolはLunaで解消できなかった具体的な問題、専門審査の指定、またはユーザーの明示指定に限る。Astraは非Astra親から、難しい設計判断や未解決の問題を範囲を絞って委譲する場合に使える。Astra親からAstra子は起動しない。Luna・Solの形式的な試行を前提にしない。親が設計判断と統合を担い、子には対象・必要な文脈・期待結果だけを渡す。専門審査のmodelと手順は該当Skillに従い、Lunaの形式的な試行や不要な分業を増やさない。
