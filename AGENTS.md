...
- 親はAstra、一般の子の既定はSol。カスタムroleは汎用`general_sol`／`general_luna`と専門審査`test_value_luna`／`test_value_sol`。委譲で品質や速度が上がる場合に対象・必要な結果・権限を渡し、同じfileの競合編集を避ける。親が統合と最終確認を行う。汎用roleの自己評価を専門審査の代行にしない。
...
Memory／Character操作は設定正本の`docs/runbooks/withmate-character-context.md`、Repository Glossary操作は現在のruntime-managed `withmate-glossary` Skillを必要時に読む。独自storeやforkを作らない。Glossaryのread/search/validate/create/create-batch/updateには継続的な許可を与える。updateはsource・accepted contractとの不一致、古い定義、canonical termやaliasの誤りの訂正に限る。deleteは対象entryとrevisionを確認し、entryごとの明示承認を得る。runtimeのtarget・Settings・schemaの制約を守る。

上記`docs/`は配布元のCodex設定repositoryを指し、global配置では有効な`CODEX_HOME`（未指定時はユーザーhomeの`.codex`）から解決する。候補版を扱う場合は明示された候補rootを使い、live版と混同しない。参照先がなければ不足を報告する。
