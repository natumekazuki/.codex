...
静的検知スクリプトの実行には `uv` を使う。Windows では WinGet から導入し、シェルを開き直して確認する。
...
WinGet を使えない場合は [uv の公式インストール手順](https://docs.astral.sh/uv/getting-started/installation/) を確認する。`uv` を導入できない環境では、`references/manual-checklist.md` を使い、静的検知を手作業で代替したことをユーザーへ明示する。
