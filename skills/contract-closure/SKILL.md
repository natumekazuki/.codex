...
- root sessionはreviewerへReview Briefを渡す前に、Candidate Definitionの必須field、identity mode、digest framing、path encoding、object type、宣言済みread-only verification recipe、draft Review Briefの必須fieldを決定的なローカルcheckで検証する。この結果を`Candidate preflight`と呼び、`verified`、`mismatch`、`validation-gap`のいずれかでtask-localに記録する。全項目が一致した`verified`だけをReview Briefへ記録し、`mismatch`または`validation-gap`ではReview Briefを発行しない。sessionの起動可否はprovider固有のdispatcherへ強制せず、reviewerが受け取った入力を独立に検証してsubstantive reviewの開始可否を決定する。
...
root sessionは`python skills/contract-closure/scripts/review_brief.py --input <review-input.json|-> [--output <review-brief.json>]`を使い、review kind、reviewer role、明示したreview target、意味入力、Candidate-bound reviewでは`candidate_snapshot.py` schema version 1の出力を検証する。`-`は標準入力のJSONを表す。builderはcurrent directoryからreview targetを推測せず、snapshotのsource identityを再生成しない。`candidate_snapshot.py verify`と同じ実装境界の結果が`verified`で、kind固有field、role、scope、Invariant引継ぎ、deadlineが整合した場合だけ`status=ready`と決定的にrenderした`reviewBrief`を発行する。`invalid`または`validation-gap`では`reviewBrief=null`と修正可能な診断を返す。builderはsession launcher、provider routing、hookを所有せず、起動済みreviewerは入力不足、期限切れ、Candidateの`mismatch`または`validation-gap`を推測で補完せずvalidation-only outputを返す。

builderの機械入力と出力は`schemaVersion: 1`のJSONを正本とし、本文は検証済みJSONからの派生物とする。Review Entry IDは入力値を維持し、省略時は`unassigned`へ正規化するだけで新規生成しない。deadlineもbuilderが既定値を作らず、有限で期限切れでない絶対時刻を入力として要求する。通常のcompleted sliceを`slice_reviewer`へ渡すCandidate非依存targeted reviewでは、review target、scope、観測可能な契約、契約または理由付き代替確認、targeted checkを`inputArtifactPreflight`で検証し、Frozen Candidateを要求しない。

Candidate-boundな`reviewBrief`には、Candidate Definitionとは別に、root preflightで検証したsnapshot schema version 1の入力を`candidateVerificationInput`として改変せず含める。これはreviewerが一時的なCandidate JSONを推測または再構築せず、`candidate_snapshot.py verify`と同じ`verify_candidate`実装境界へ渡すための実行入力であり、新しいsource identityやsnapshotではない。reviewerはこの入力と明示target rootを使って独立検証し、`verified`の場合だけsubstantive reviewへ進む。
...
- reviewerへsubstantive reviewを依頼する前に、root sessionがCandidate preflightとReview Briefのローカル検証を完了する。reviewerへはreview kindに対応するReview Briefだけを渡し、実装者の結論を確定事実として渡さない。session起動自体をprovider固有hookで強制せず、reviewerが受信入力とCandidateを独立検証してsubstantive reviewへ進めるか判定する。
