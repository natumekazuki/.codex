...
完了に必要な実行済みcheck、未実行check、validation gapだけを報告する。新規・意味変更testの価値審査の適用条件と、抽出exit `0`・判定`ACCEPT`の扱いは`AGENTS.md` §4および`review-test-value`へ従う。既存testを変更せず実行するだけの作業では、このworkflowや審査を追加しない。
