...
- 宣言開始行が削除・置換された旧recordは、未変更の旧宣言行が同じ現行宣言に属する場合だけ位置による対応を認める。metadataや閉じ行が隣接recordと共通でも、それだけで存続を確定しない。対応先の宣言がなく、開始行の変更hunkにも現行宣言がなければ`DELETED`として旧recordを保持する。
