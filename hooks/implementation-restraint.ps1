...
- 互換性対応なしでは現在の要求が成立せず、互換性を明示的に要求するaccepted contract、具体的な外部consumer、またはmigration要件を確認できる場合だけ、必要性、対象範囲、trade-off、撤去条件を示し、その作業に対するユーザーの明示承認を得てから実装する。既存codeやtest、過去の挙動、有益そうという推測は必要性または承認の根拠にしない。承認がなく、互換性対応なしではaccepted contractを満たせない場合は、部分的に実装せずblockedとして報告する。
