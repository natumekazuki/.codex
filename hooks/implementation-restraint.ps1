...
- 共通AGENTS.mdの「条件付き参照」に従い、実装を伴わない課題・対応予定の管理でも、~/.codex/docs/guides/development.mdと対象repositoryが明示する関連運用文書を読む。開発・review・リリース範囲の確定・統合・公開判断等の読込条件も同入口に従う。バグ・後追い残件の管理先または互換性境界が未定義なら実装変更を開始しない。その他の不足による停止範囲は詳細に従う。必須確認と権限を満たして統合し、現在の変更や依存先を非ブロッカーの全件修正待ちにしない。
