...
- AGENTS.mdの「統合優先・修正分離」に従う。全review指摘の修正をmerge条件にせず、非ブロッカーはプロジェクトで定義した管理先へ記録して別タスクとして扱う。管理先が未定義なら実装変更を開始せず確認する。必須確認と権限を満たして統合し、現在の変更や依存先を後追い修正の着手・完了待ちにしない。
