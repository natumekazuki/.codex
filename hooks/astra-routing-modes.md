...
`PreToolUse`は専用roleまたは明示された`gpt-6-astra`を検出し、mode、turn、共有枠、同時実行、manual grantを満たさない呼出しを拒否する。`SubagentStart`と`SubagentStop`で実行中のAstraを追跡し、`SessionStart`では既存stateを再注入する。state破損時はAstra投入と、対象modelを判定できない既存agentへのfollow-upを拒否する。新しいSol / Luna作業は継続できる。破損状態は次のturnで自動解除しない。該当sessionでAstraが動作中でないことを確認して対象state fileを削除すると、次のuser promptで再生成される。
