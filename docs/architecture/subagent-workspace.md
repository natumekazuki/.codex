...
`test_value_luna`と`test_value_sol`の審査は別の専門境界を持つ。通常の子をforkして入力隔離の代わりにせず、[review-test-value](../../skills/review-test-value/SKILL.md)の有効な実行経路へ渡す。metadataだけのphaseへ本文、親履歴、一般hookを混入させない。
