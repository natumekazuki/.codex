...
履歴の保存先はADRと各repositoryの`docs/releases/`のリリースノートに限定する。それ以外の文書・ソースコードには現行の情報だけを残す。ADRの本文保持・適用状態の明示と、リリースノートから過去の詳細へのcommit固定リンクは[設計判断・文書化](../guides/design-decisions.md)に従う。
