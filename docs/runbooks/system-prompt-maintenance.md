...
6. モデルまたは取得規約が変わった場合は、`docs/system-prompt/README.md` を現行の構成に合わせます。役割を終えたスナップショットと訳を削除し、参照・文書の更新とまとめてコミットします。公開時の変更記録は`docs/releases/`に残し、過去の詳細へのリンクをGit commitの完全ハッシュへ固定します。
