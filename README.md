...
| 条件 | 確認する行動 |
| --- | --- |
| バグ・後追い残件の管理先または互換性境界が未定義／両方とも定義済み | 前者は実装を開始せず、調査・推奨案・草案から採否を確認する。後者は既存定義を利用する |
...
既存の着手停止条件は前表でも確認する。文書上の整合、ケースに対する判断、runtimeでの読込・動作の結果を分けて既存の作業報告・PR等へ残す。実施していないlive配置・新規sessionでの挙動を検証済みとせず、新しい恒久test・同期機構・証跡台帳は追加しない。
