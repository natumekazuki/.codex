...
- `vMAJOR.MINOR.PATCH`形式のGitタグが指すcommitをリリース状態とし、配布版の識別に使う。互換維持の対象はそのタグのリリースノートに記載した利用者向け契約であり、実装全体ではない。契約の破壊的変更はmajor versionを上げ、互換修正はminorまたはpatch versionを上げて新しいタグへ記録する。
...
| `docs/guides/` | 課題・対応予定・リリース運用の共通観点と開発・統合判断、設計判断・文書化、変更test、UI、WithMate操作の作業別詳細 |
| `agents/` | 汎用のカスタムrole |
...
| 条件 | 確認する行動 |
| --- | --- |
| 管理先または互換性境界が未定義で、変更の判断に影響する／影響しない | 前者は依存する変更・引継ぎを止め、調査・推奨案・草案から採否を確認する。後者は実際の差分と依存先から独立性を説明して進める |
| 配備済み画面の配置・文言・入力手順を変える／業務承認・計上条件・外部送信内容を変える | 前者は保護対象への影響を確認し、配備UIと異なることだけでは止めない。後者はUI変更でも契約変更として確認する |
| タグのないRC・STGの保持データや適用済みmigrationを変更する | タグの有無で保護を免除せず、データ破棄や適用済みmigrationの書換えを許可しない |
| 境界の不足がschema変更の可否に影響するが、別の文書調査は独立している | schema変更を止めて境界の草案・採否確認を進め、独立した調査は継続する |
| repository内Markdownが残件管理先として定義済み／管理先未定義で残件を伴わない独立変更 | 前者は外部サービス未準備を理由に止めない。後者は実装を一律に止めず、残件が生じたら管理先の確定・登録状況を明示する |
...
この領域のルール変更時は、入口・本書・共通guide・関係する現役の参照やhookを照合し、入口への管理手順の重複、特定プロジェクトの運用の流用、一律停止条件の拡張がないか確認する。次は変更の検証例であり、日々の全タスクへ追加するチェックリストではない。

| 条件 | 確認する行動 |
| --- | --- |
| 実装を伴わない課題管理／プロジェクトの管理ルールが別文書にある | 読む条件から共通・固有の正本へ到達する。入口への転記や無関係な全文書読込を行わない |
| 正式タグ未作成のMilestoneへ割り当てる／開発ブランチで対応版未決定のバグを発見する | 前者は採用済み規則と権限で予定を扱い、後者は発生・調査対象から修正版を推定しない |
| バージョン単位の管理を採用しない／定義済み運用で予定未指定を許容する | ツール等の導入や一律停止を要求しない。リリース範囲確認では関連する未指定課題も確認する |
| 既存の着手条件は成立するが、対応予定の管理方法が未定義 | 不足に依存する判断・操作だけを保留し、独立した作業は進める |
| 同じ予定単位に非ブロッカーが残る／未完了のまま予定単位を閉じようとする | 現在の統合を一律停止せず、完了条件・正当な延期・権限を確認する。元の依頼の未完了を隠すクローズ・移動はしない |
| 課題は閉じているが、対象ブランチへの反映が未確認 | PR・commit等で反映と検証の根拠を確認し、反映済み・公開済みとは推定しない |
| 管理先は定義済みだがread-onlyまたは外部write未承認／判断・操作が承認・委譲済み | 前者は提案にとどめ、後者は承認範囲で進める。形式的な再確認や無断の範囲拡大をしない |
| 参照または更新が失敗する | 未確認・未実施を明示し、残件ゼロ・登録済み・引継ぎ済みとしない |

変更影響に応じた停止条件は前表でも確認する。文書上の整合、ケースに対する判断、runtimeでの読込・動作の結果を分けて既存の作業報告・PR等へ残す。実施していないlive配置・新規sessionでの挙動を検証済みとせず、新しい恒久test・同期機構・証跡台帳は追加しない。
