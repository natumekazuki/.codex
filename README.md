...
統括と機能別セッションで開発する場合は、主要動作と後続が依存する契約を確認し、実装・必須確認・必要な初回reviewを行う。統合ブロッカーだけを解消し、残件を引き継いで権限のある統合先へmergeする。統合後の必要な確認が成立すれば、後続機能を着手可能にする。独立した修正セッションの起動・完了を、その待ち条件へ追加しない。
...
| Aの主要動作・必須確認は成立し、補助表示の不具合が残る。BがAの統合待ち | Aを引継ぎ・統合・必要確認後、Bを着手可能にし、空き枠があれば開始する。Aの非ブロッカー修正を待たない |
...
| 後追い修正が現行バージョンに間に合わない／修正キューが空になる | 非ブロッカーだけで公開を延期しない。空のキューを埋めるための探索を始めない |
